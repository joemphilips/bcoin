...
class Chain extends AsyncEmitter {
  ...
  async fillPSBT(psbt) {
    const view = await this.getCoinView(psbt.tx);
    for (const i in psbt.tx.inputs) {
      const input = psbt.tx.inputs[i];

      const coin = view.getOutput(input.prevout.hash);
      if (coin)
        psbt.inputs[i].witnessUTXO = coin;

      const tx = await this.getTX(input.prevout.hash);
      if (tx)
        psbt.inputs[i].nonWitnessUTXO = tx;
    }
  }
