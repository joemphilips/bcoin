...
class RPC extends RPCBase {
  ...
  init() {
    ...
    this.add('walletprocesspsbt', this.walletProcessPSBT);
    this.add('walletcreatefundedpsbt', this.walletCreateFundedPSBT);
    this.add('walletfundtocrowdfundingpsbt', this.walletFundToCrowdFundingPSBT);
  }
  ...
  async walletProcessPSBT(args, help) {
    if (help || args.length < 1 || 4 < args.length)
      throw new RPCError(errs.MISC_ERROR, 'walletprocesspsbt');
    const valid = new Validator(args);
    const psbt64 = valid.str(0);
    const sign = valid.bool(1, true);
    const sighash = valid.str(2, 'ALL');
    const bip32 = valid.bool(3, false);
    if (Object.keys(Script.hashType).indexOf(sighash) === -1) {
      throw new RPCError(errs.INVALID_PARAMS, 'Invalid sighash type');
    }
    let psbt;
    try {
      psbt = PSBT.fromRaw(psbt64, 'base64');
    } catch (e) {
      throw new RPCError(errs.PARSE_ERROR, 'Failed to decode psbt');
    }
    const complete = await this.wallet.fillPSBT(psbt, {bip32, sighash, sign});
    return {
  ...
  async walletFundToCrowdFundingPSBT(args, help)
  {
    if (help || args.length < 1 || 2 < args.length )
      throw new RPCError(errs.MISC_ERROR,
        'walletfundtocrowdfundingpsbt psbt (num to fund)'
        );

    const wallet = this.wallet;
    const valid = new Validator(args);
    const psbt64 = valid.str(0);
    const fundNum = valid.i16(1, 1);
    let psbt;
    try
    {
      psbt = PSBT.fromRaw(psbt64, 'base64');
    ...
    {
      throw new RPCError(errs.PARSE_ERROR, 'failed decode psbt');
    }

    const psbtresult = await wallet.fundPSBT(psbt, fundNum);
    console.log("result is ")
    console.log(psbtresult)
    console.log(psbtresult.tx.inspect())
    return {
      psbt: psbtresult.toRaw().toString('base64'),
    };
  }

  async walletCreateFundedPSBT(args, help) {
    if (help || args.length < 2 || 5 < args.length) {
      throw new RPCError(errs.MISC_ERROR,
    ...
    const locktime = valid.u32(2, 0);
    const options = valid.obj(3);

    if (!inputs)
      throw new RPCError(errs.TYPE_ERROR, 'Invalid inputs');
    if (!outputs)
      throw new RPCError(errs.TYPE_ERROR, 'Invalid outputs');
    ...
    for (const key of Object.keys(outputs)) {
      if (key === 'data') {
        ...
        if (!value)
          throw new Error(errs.TYPE_ERROR, 'Invalid null data');
    ...
    const fundOptions = {};
    const fillPSBTOptions = {sign: false};
    if (options) {
      const valid = new Validator(options);

      fundOptions.rate = valid.ufixed('feeRate', 8);
      fundOptions.changeAddress = valid.str('changeAddress');
      fundOptions.lockUnspents = valid.bool('lockUnspents');
      fundOptions.subtractFee = valid.bool('subtractFeeFromOutputs');
      fundOptions.blocks = valid.u8('conf_target');

      fillPSBTOptions.bip32 = valid.bool('bip32derives', false);
    }

    await wallet.fund(tx, fundOptions);

    const psbt = PSBT.fromMTX(tx);
    await wallet.fillPSBT(psbt, fillPSBTOptions);
    return {
      psbt: psbt.toRaw().toString('base64'),
      fee: tx.getFee(),
      changepos: tx.chaingeIndex
    };
  }
}
