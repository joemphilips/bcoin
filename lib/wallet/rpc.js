...
class RPC extends RPCBase {
  ...
  walletProcessPSBT(args, help) {
    ...
    const sighashType = valid.str(2, 'ALL');
    const bip32derivs = valid.bool(3, false);
    if (Object.keys(Script.hashType).indexOf(sighashType) === -1) {
      throw new RPCError(errs.PARSE_ERROR, 'Invalid sighash type');
    }
    let psbt;
    try {
      psbt = PSBT.fromRaw(Buffer.from(psbt64, 'base64'));
    } catch (e) {
      throw new RPCError(errs.PARSE_ERROR, 'Failed to decode psbt');
    }
    this.wallet.fillPSBT(psbt);
  }

  walletCreateFundedPSBT(args, help) {
