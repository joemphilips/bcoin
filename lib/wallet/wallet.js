...
const KeyOriginInfo = require('../hd/keyorigin.js');
...
class Wallet extends EventEmitter {
   ...
   * The reason this logic is not implemented as PSBT's method is because we must
   * provide whole TX in case input is not witness.
  ...
  async fillPSBT(psbt, options) {
    const unlock = this.fundLock.lock();
    try {
  ...
  async _fillPSBT(psbt, options) {
    ...
    const bip32 = options.bip32 || true;

    // 1. set all previous tx info if the wallet has any
    for (const i in psbt.tx.inputs) {
      const psbtin = psbt.inputs[i];

      if (psbtin.sighash === -1)
        psbtin.sighash = sighash;

      const {outpoint} = psbt.tx.inputs[i];

      const prevtx = await this.getTX(outpoint);
      if (!prevtx)
        continue; // wallet has no information about this input.
      psbtin.nonWitnessUTXO = prevtx;
      const coin = prevtx[outpoint.index];
      psbtin.witnessUTXO = coin;
    }

    // 2. template scripts and sign.
    const rings = this.deriveInputs(psbt.tx);
      for (const key of rings)
        psbt.sign(key);

    // 3. set output info
    for (const i in psbt.tx.outputs) {
      const out = psbt.tx.outputs[i];
      const psbtout = psbt.outputs[i];
      ...
      if (bip32) {
        const path = await this.getPath(address);
        const {account, branch, index} = path;
        const coinType = this.network.keyPrefix.coinType;
        psbtout.keyInfo.set(
          key.getPublicKey(),
          new KeyOriginInfo({
            path: [44, coinType, account, branch, index],
            fingerPrint: this.master.key.fingerPrint
          })
        );
      }

      psbt.scriptOutput(i, key);
    }
    return psbt;
  };
