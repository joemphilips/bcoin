...
class Wallet extends EventEmitter {
  ...
  async fundPSBT(psbt, fundNum) {
    const unlock = await this.fundLock.lock();
    try {
      return this._fundPSBT(psbt, fundNum);
    } finally {
      unlock();
    }
  }

  async _fundPSBT(psbt, options) {
    const account = options.account;
    const value = options.value || psbt.tx.outputs[0].value / 5;
    for (const psbtin of psbt.inputs)
      assert(psbtin.sighash & Script.hashType.ANYONECANPAY == Script.hashType.ANYONECANPAY, 'crowd funding psbt must have ANYONECANPAY flag');

    const fee = psbt.getFee();
    assert(fee !== -1, 'can not calculate psbt fee! Check psbt has complete UTXO for inputs');
    if (fee > 0) {
      this.logger.debug("PSBT is already funded.");
      return true; // already funded
    }

    // 1. create tx for input.
    const mtx = new MTX();
    const payAddressKey = await this.createReceive(account);
    const payOutput = new Output({address: payAddressKey.getAddress(), value});
    mtx.addOutput(payOutput);

    await this.fund(mtx);
    assert(mtx.inputs.length > 0, 'failed to fund mtx');

    const inputKeys = await this.deriveInputs(mtx);
    mtx.template(inputKeys);
    mtx.sign(inputKeys);

    // 2. psbt
    const cfmtx = MTX.fromTX(psbt.tx);
    const key = await this.getKey(payAddressKey.getAddress());
    console.log(key)
    cfmtx.addTX(mtx, 0);
    // await this.sign(cfmtx);
    const psbt2 = PSBT.fromMTX(cfmtx);
    for (let i = 0; i < psbt.inputs.length; i++)
      psbt2.inputs[i] = psbt.inputs[i];
    for (let i = 0; i < psbt.outputs.length; i++)
      psbt2.outputs[i] = psbt.outputs[i];

    const finalS = cfmtx.inputs[0].script;
    const finalW = cfmtx.inputs[0].witness;
    const tx = mtx.toTX();
    console.log(finalS)
    console.log(finalW)
    console.log(tx)
    // psbt2.inputs[psbt2.inputs.length - 1].finalScriptSig = finalS
    // psbt2.inputs[psbt2.inputs.length - 1].finalScriptWitness = finalW;
    // psbt2.inputs[psbt2.inputs.length - 1].nonWitnessUTXO = tx;

    return psbt2;
  }

  /**
   * Fund (but do not sign) a psbt.
   * @param {PSBT} psbt - psbt to fund.
   * @param {Number?} options.sighash
   * @param {Boolean?} options.sign - if sign to the psbt or not.
   * @param {Boolean?} options.bip32 - include derivation path. default is true.
   * @returns {PSBT} - updated psbt
   */

  async fillPSBT(psbt, options) {
    const unlock = await this.fundLock.lock();
    try {
      return this._fillPSBT(psbt, options);
    } finally {
      unlock();
    }
  }

  async _fillPSBT(psbt, options) {
    options = options || {};
    const sighash = options.sighash || Script.hashType.ALL;
    const bip32 = options.bip32 === false ? false : true;

    // 1. set all previous tx info if the wallet has any
    // why not use `this.template()` ? because it requires tx to be mutable.
    const ringsForInput = [];
    for (let i = 0; i < psbt.tx.inputs.length; i++) {
      const psbtin = psbt.inputs[i];

      if (psbtin.sighash === -1)
        psbtin.sighash = sighash;

      const {prevout} = psbt.tx.inputs[i];

      const record = await this.getTX(prevout.hash);
      if (!record || !record.tx)
        continue; // wallet has no information about this input.
      psbtin.nonWitnessUTXO = record.tx;
      const coin = record.tx.outputs[prevout.index];
      psbtin.witnessUTXO = coin;
      ringsForInput.push(await this.getKey(coin.script.getAddress()));
    }

    const ringsForOutput = [];
    for (const k of psbt.tx.outputs) {
      const key = await this.getKey(k.script.getAddress());
      if (key)
        ringsForOutput.push(key);
    }

    // 2. template scripts and sign.
    psbt.update([...ringsForInput, ...ringsForOutput], bip32);
    if (options.sign)
      psbt.sign(ringsForInput);
  ...
  async createFundedPSBT(options, force, passphrase) {
    const mtx = await this.createTX(options, force);
    this.sign(mtx, passphrase);
    const psbt = PSBT.fromMTX(mtx);
    return this.fillPSBT(psbt);
  }
