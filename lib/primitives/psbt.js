...
class PSBT {
  ...
  globalsFromReader(br) {
    ...
    // We know second key for global KVMap should be a separator.
    // So assert here to make sure nothing went wrong.
    key = br.readVarBytes();
    ...
    while (!key.equals(Buffer.from(''))) {
      ...
      switch(key.readUInt8()) {
        ...
        case PSBT_IN_PARTIAL_SIG:
          pubkey = key.slice(1);
          assert(!this.signatures.has(pubkey), 'duplicate key for partial signature');
          assert(
