...
class PSBT {
  ...
  fromTX(tx, view) {
    ...
    assert(view, 'must pass coinview');
    this.tx = tx.clone();
    for (const i in this.tx.inputs) {
      ...
      const stack = []; // holder for items which could be signature.
      // p2sh
      ...
      // move signatures and pubkeys if already present in tx.
      const [required, pubkeys] = nextScript.matchMultisig();
      if (required > 0) {
        for (const p of pubkeys) {
          for (const item of stack) {
            if (common.isSignatureEncoding(item) &&
                tx.checksig(i, nextScript, coin.value, item, p, sighashV)) {
  ...
  checkSanity() {
    ...
    for (const i in this.inputs) {
      ...
      if (!psbtin.nonWitnessUTXO.isNull()) {
        if (!psbtin.nonWitnessUTXO.hash().equals(input.prevout.hash)) {
  ...
  sign(ring) {
    ...
    for (const i in this.inputs) {
      ...
      if (!this.inputs[i].hasOwnOutputs(ring, txin))
        continue;
      this.scriptInput(i, ring);
      this.signInput(i, ring);
      total++;
    }
  ...
  /**
   * create input script for each inputs if possible.
   * Also set bip32 derivation path for both inputs and outputs.
   * only If the ring is `WalletKey`
  ...
  scriptInput(index, ring) {
    ...
    if (wit.isScripthashInput()) {
      const witScript = wit.getRedeem();
      psbtin.witness = witScript;
    }
    if (dummyInput.script.code.length > 0)
      psbtin.redeem = dummyInput.script.getRedeem();

    // 2. bip32 derivation path.
    if (ring.keyType === Path.types.HD) {
  ...
  signInput(index, ring) {
    ...
    if (psbtin.complete)
      ...
      return true;
    const signNonWitness = !psbtin.nonWitnessUTXO.isNull();
    const signWitness = psbtin.witnessUTXO.script.code.length > 0;
    assert(signNonWitness || signWitness, 'psbt is not templated !');
    if (signNonWitness) {
      const prev = psbtin.nonWitnessUTXO;
      assert(prev.hash().equals(txin.prevout.hash),'txid mismatch');

      const output = prev.outputs[txin.prevout.index];
      const script = output.script;
      const value = output.value;
      const redeem = psbtin.redeem;
      // p2sh
      if (redeem.code.length > 0) {
        assert(redeem.isScripthashInput(),'redeem seems to be non-p2sh');
        const redeembuf = redeem.getScripthashInput();
        assert(
          script.getScripthash() === sha256.digest(redeembuf),
          'redeem script hash does not match previous scriptPubKey'
        );
        return this.createSignature(ring, index, redeem, value, false);
      // p2pkh
      } else {
        return this.createSignature(ring, index, script, value, false);
      }

    // segwit
    } else if (signWitness) {
      // Check if it is p2sh-nested segwit
      let scriptToSign;
      const coin = psbtin.witnessUTXO;
      const redeem = psbtin.redeem;
      const witness = psbtin.witness;
      if (redeem.code.length > 0) {
        // Check if scriptPubkey is actually a correct script hash.
        assert(
          coin.script.getScripthash()
            .equals(sha256.digest(redeem.getScripthashInput())),
          'scriptPubKey and redeem script hash does not match'
        );
        scriptToSign = psbtin.redeem;
      } else {
        scriptToSign = coin.script;
      }

      const value = psbtin.witnessUTXO.value;

      if (scriptToSign.isWitnessScripthash()) {
        assert(
          coin.script.getScripthash()
            .equals(sha256.digest(witness.getScripthashInput())),
          'scriptPubKey and witness script hash does not match'
        );
      }
      return this.createSignature(ring, index, scriptToSign, value, true);
    }
    return false;
  }

  createSignature(ring, index, prev, value, witness) {
    const psbtin = this.inputs[index];
    const hashType = psbtin.sighash > 0 ? psbtin.sighash : null;
    const version = witness ? 1 : 0;
    const sig = this.tx.signature(
      index,
      prev,
      value,
      ring.privateKey,
...
class PSBTInput {
  ...
  checkSanity() {
    if (
      this.witnessUTXO.script.code.length > 0 && !this.nonWitnessUTXO.isNull()
    )
      ...
      switch(key.readUInt8()) {
        ...
        case PSBT_IN_BIP32_DERIVATION:
          ...
          assert(
            ...
            'pubkey size for bip32 derivation path is not correct.'
            );
