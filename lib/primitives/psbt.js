...
class PSBT {
  ...
  scriptInput(index, ring, bip32) {
    ...
    const redeem = dummyInput.script.getScripthashInput();
    if (redeem)
  ...
  scriptOutput(index, ring, bip32) {
    const out = this.ix.outputs[index];
  ...
  signInput(index, ring, finalize) {
    ...
    if (nextScript.isScripthash()) {
      if (!psbtin.redeem.hash160().equals(coin.script.getScripthash()))
        throw new Error('psbt has no good redeem script');
      nextScript = psbtin.redeem;
    }

    // p2wsh
    if (nextScript.isWitnessScripthash()) {
      if (!psbtin.witness.forWitness().equals(nextScript))
        throw new Error('witness mismatch');
      nextScript = psbtin.witness;
      witness = true;
    }
    ...
    const success = this.createSignature(
      ring,
      index,
      nextScript,
      coin,
    ...
    const [m, pubkeys] = nextScript.matchMultisig();
    if (m === -1)
      return success;

    const sigs = [];
    for (const k of pubkeys) {
      const sig = psbtin.signatures.get(k);
      if (sig)
        sigs.push(sig);
    }
    if (sigs.length < m)
    ...
    if (witness) {
      ...
      scriptWitness.push(nextScript.toRaw());
      psbtin.finalScriptWitness = scriptWitness;
      return 1;
    }
    ...
    scriptSig.pushData(psbtin.redeem.toRaw());
    psbtin.finalScriptSig = scriptSig;
    return 1;
  }
  ...
  finalize() {
    for (let i = 0; i < this.inputs.length; i++) {
      assert(this.inputs[i].tryFinalize(), `failed to finalize input ${i}`);
    }
    this.unknown.clear();
  };
...
class PSBTInput {
  ...
  tryFinalize() {
    let m;
    let pubkeys;
    let prev = this.getOutput();
    if (prev.isScripthash()) {
      if (this.redeem.isProgram())
        prev = this.redeem;
    }

    if (this.witness.isScripthash()) {
      [m, pubkeys] = this.witness.matchMultisig();

      const sigs = [];
      for (const k of pubkeys) {
        sigs.push(this.signatures.get(k));
      }
      if (sigs.length < m)
        return false;
    }
