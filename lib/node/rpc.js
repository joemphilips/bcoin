...
const PSBT = require('../primitives/psbt');
...
class RPC extends RPCBase {
  ...
  init() {
    ...
    this.add('decodepsbt', this.decodePSBT);
    this.add('combinepsbt', this.combinePSBT);
    this.add('finalizepsbt', this.finalizePSBT);
    this.add('createpsbt', this.createPSBT);
    this.add('converttopsbt', this.convertToPSBT);
  }
  ...
  async decodePSBT(args, help) {
    if (help || args.length !== 1)
      throw new RPCError(errs.MISC_ERROR, 'decodepsbt');
    const valid = new Validator(args);
    const str = valid.str(0, '');
    let psbt;
    try {
      psbt = PSBT.fromRaw(str, 'base64');
    } catch (e) {
      throw new RPCError(errs.PARSE_ERROR, 'failed to parse psbt');
    }
    return this.psbtToJSON(psbt);
  }

  async combinePSBT(args, help) {
    if (help || args.length !== 1)
      throw new RPCError(errs.MISC_ERROR, 'combinepsbt');
    const valid = new Validator(args);
    const strs = valid.array(0);
    const psbts = [];
    for (let i = 0; i < strs.length; i++) {
      const str = strs[i];
      try {
        psbts.push(PSBT.fromRaw(str, 'base64'));
      } catch (e) {
        throw new RPCError(errs.PARSE_ERROR, `failed to parse psbt ${i}`);
      }
    }
    const psbt = psbts.reduce((a, b) => a.combine(b));
    return psbt.toRaw().toString('base64');
  }

  async finalizePSBT(args, help) {
    if (help || args.length !== 2)
      throw new RPCError(errs.MISC_ERROR, 'finalizepsbt');
    const valid = new Validator(args);
    const str = valid.str(0);
    const extract = valid.bool(1, true);
    let psbt;
    try {
      psbt = PSBT.fromRaw(str, 'base64');
    } catch (e) {
    ...
    const complete = psbt.finalize();
    let tx;
    if (extract)
      tx = psbt.toTX();
    return {
      psbt: psbt.toRaw().toString('base64'),
      hex: extract ? tx.toRaw().toString('hex') : '',
      complete
    };
  }
  ...
  async createPSBT(args, help) {
    if (help || args.length < 2 || 4 < args.length)
      throw new RPCError(errs.MISC_ERROR, 'createpsbt');
    const valid = new Validator(args);
    const inputs = valid.array(0);
    const outputs = valid.obj(1);
    const locktime = valid.u32(2);

    if (!inputs || !outputs) {
      throw new RPCError(errs.TYPE_ERROR,
        'Invalid parameters (inputs and outputs).');
    }
    ...
    const sends = new Validator(outputs);
    const uniq = new Set();
    ...
    for (const key of Object.keys(outputs))  {
      if (key === 'data') {
        const value = sends.buf(key);
        if (!value)
          throw new RPCError(errs.TYPE_ERROR, 'Invalid data');

        const output = new Output();
        output.value = 0;
        output.script.fromNulldata(value);
        tx.outptus.push(output);
        continue;
      }
      const addr = parseAddress(key, this.network);
      const b58 = addr.toString(this.network);
      ...
      if (uniq.has(b58))
        throw new RPCError(errs.INVALID_PARAMETER, 'Duplicated address');

      uniq.add(b58);

      const value = sends.ufixed(key, 4);
      if (value === null)
        throw new RPCError(errs.TYPE_ERROR, 'Invalid output value');
    ...
    const psbt = PSBT.fromMTX(tx);
    return psbt.toRaw().toString('base64');
  }

  // TODO: shuold take `permitsig` option
  async convertToPSBT(args, help) {
    if (help || args.length < 1 || 3 < args.length )
      throw new RPCError(errs.MISC_ERROR, 'createpsbt');

    const valid = new Validator(args);
    const hex = valid.str(0);
    let tx;
    let psbt;
    try {
      tx = TX.fromRaw(hex, 'hex');
    } catch (e) {
      throw new RPCError(errs.PARSE_ERROR, 'failed to parse tx');
    }
    try {
      psbt = PSBT.fromTX(tx);
    } catch (e) {
      throw new RPCError(errs.MISC_ERROR, 'failed to convert to psbt');
    }
    return psbt.toRaw().toString('base64');
  }
  ...
  psbtToJSON(psbt) {
    ...
    for (const [k, v] of psbt.unknown)
      unknown[k.toString('hex')] = v.toString('hex');

    const inputs = [];
    for (const psbtin of psbt.inputs) {
      inputs.push(this.psbtInputToJSON(psbtin));
    };

    const outputs = [];
    for (const psbtin of psbt.inputs) {
      outputs.push(this.psbtOutputToJSON(psbtin));
    };

    return {
      tx: this.txToJSON(psbt.tx),
      unknown,
      inputs,
      outputs
    };
  ...
  psbtInputToJSON(psbtin) {
    const json = {};
    if (psbtin.nonWitnessUTXO)
      json.non_witness_utxo = this.txToJSON(psbtin.nonWitnessUTXO);

    if (psbtin.witnessUTXO) {
      const tmp = {};
      const v = psbtin.witnessUTXO;
      tmp.amount = v.value;
      tmp.scriptPubKey = this.scriptToJSON(v.script, true);
      json.witness_utxo = tmp;
    }

    if (psbtin.signatures.size > 0) {
      const tmp = {};
      for (const [k, v] of psbtin.signatures) {
        tmp[k.toString('hex')] = v.toString('hex');
      };
      json.partial_signatures = tmp;
    }

    if (psbtin.sighash !== -1)
      json.sighash = psbtin.sighash;

    if (psbtin.redeem.code.length > 0)
      json.redeem_script = this.scriptToJSON(psbtin.redeem, true);

    if (psbtin.witness.code.length > 0)
      json.witness_script = this.scriptToJSON(psbtin.witness, true);

    if (psbtin.keyInfo.size > 0) {
      const tmp = {};
      for (const [k, v] of psbtin.keyInfo) {
        tmp[k.toString('hex')] = v.toString('hex');
      }
      json.bip32_derivs = tmp;
    }

    if (psbtin.finalScriptSig) {
      json.final_scriptsig = this.scriptToJSON(psbtin.finalScriptSig);
    }

    if (psbtin.finalScriptWitness) {
      const tmp = [];
      for (const w of psbtin.finalScriptWitness.items) {
        tmp.push(w.toString('hex'));
      }
      json.final_scriptwitness = tmp;
    }

    if (psbtin.unknown.size > 0) {
      const tmp = {};
  ...
  psbtOutputToJSON(psbtout) {
    const json = {};
    if (psbtout.redeem.code.length > 0)
      json.redeem_script = this.scriptToJSON(psbtout.redeem, true);

    if (psbtout.witness.code.length > 0)
      json.witness_script = this.scriptToJSON(psbtout.witness, true);

    if (psbtout.keyInfo.size > 0) {
      const tmp = {};
      for (const [k, v] of psbtout.keyInfo) {
        tmp[k.toString('hex')] = v.toString('hex');
      }
      json.bip32_derivs = tmp;
    }

    if (psbtout.unknown.size > 0) {
      const tmp = {};
      for (const [k, v] of psbtout.unknown) {
        tmp[k.toString('hex')] = v.toString('hex');
      }
      json.unknown = tmp;
    }
